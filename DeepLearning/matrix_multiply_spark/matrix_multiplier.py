...
class MatrixMultiplier:
    ...
    def __init__(self,
        ...
        if self.D != len(self.df_cols):
            raise RuntimeError('Inner dimensions do not match (spark_df = {}, np_matrix = {})'.format(len(self.df_cols),
                                                                                                      self.D))
        ...
        if isinstance(bias,list) or isinstance(bias,np.ndarray):
            if len(bias) == self.K:
              ...
              raise RuntimeError('Bias and output dimensions do not match (bias = {}, output = {})'.format(len(bias),
                                                                                                           self.K))
    ...
    def _multiply(self):
        ...
        for i in range(self.K):
            ...
            if self.activation == None:
                new_cols.append('({} + {})'.format(' + '.join(new_col),self.bias[i]) + ' as col_%s' %i )
            elif self.activation == 'sigmoid':
                new_cols.append('1/(1 + exp(-({} + {})))'.format(' + '.join(new_col),self.bias[i]) + ' as col_%s' %i )
            elif self.activation == 'tanh':
                new_cols.append('tanh({} + {})'.format(' + '.join(new_col),self.bias[i]) + ' as col_%s' %i )
            elif self.activation == 'relu':
                new_cols.append('case when {cols} + {bias} > 0 then {cols} + {bias} else 0 end'.format(cols=' + '.join(new_col),bias=self.bias[i]) + ' as col_%s' %i )
                
        columns = ',\n\t'.join(new_cols)
        self.df.registerTempTable('table')
